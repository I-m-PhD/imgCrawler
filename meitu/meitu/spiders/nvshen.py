...
class NvshenSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        pul = []
        mpn = int(re.sub(pattern='([^0-9])', repl='', string=response.xpath('//*[@id="pages"]/a[text()="尾页"]/@href').extract_first()))
    ...
    async def parse_i(self, response):
        ...
        async with async_playwright() as p:
            b = await p.chromium.launch(headless=True)  # open browser in headless mode
            t = await b.new_page()  # open new tab
            await t.goto(response.url)
            s = await t.inner_text('//*[@id="diggnum"]')  # Score
            await b.close()
        q = re.sub(pattern='([^0-9])', repl='', string=response.xpath('/html/body/div[3]/div[1]/span/text()').extract_first())  # Quantity
